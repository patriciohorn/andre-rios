...
const ReviewCard = ({ img }: { img: string }) => {
  return (
    <figure
      className={cn(
        "bg-primary-50 relative cursor-pointer p-4"
        // light styles
      )}
    >
...
export function LogoMarque() {
  return (
    <div className="relative flex w-full flex-col items-center justify-center overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-y-0 left-0 w-1/4 bg-gradient-to-r from-primary-50"></div>
      <div className="pointer-events-none absolute inset-y-0 right-0 w-1/4 bg-gradient-to-l from-primary-50"></div>
    </div>
  );
}
